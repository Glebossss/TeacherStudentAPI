...
function loadPages() {
    $("#pages").empty();
    $.getJSON('/confirmedactivitiesforstudent/count', function (data) {
...
function loadData(pages) {
    $("#data > tbody").empty();
    $.getJSON('/confirmedactivitiesforstudent?page=' + pages, function (data) {
        var i;
        for (i = 0; i < data.length; i++) {
            $('#data > tbody:last-child').append(
                $('<tr>')
                    ...
                    .append($('<td>').append(data[i].studentEntity))
                    .append($('<td>').append(data[i].teacherEntity))
                    .append($('<td>').append(data[i].time))
                    .append($('<td>')
                        .append($('<button onclick="del_func(this.value)" > ')
                            .attr('id', i)
