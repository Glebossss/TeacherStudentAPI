...
function loadPages() {
    $("#pages").empty();
    $.getJSON('/unconfirmedactivitiesforteacher/count', function (data) {
...
function loadData(pages) {
    $("#data > tbody").empty();
    $.getJSON('/unconfirmedactivitiesforteacher?page=' + pages, function (data) {
        var i;
        for (i = 0; i < data.length; i++) {
            $('#data > tbody:last-child').append(
                $('<tr>')
                    ...
                    .append($('<td>')
                    .append($('<button onclick="add_func(this.value)" > ')
                        .attr('id', i)
                        .attr('value', data[i].id)
                        .append('Add')))
                    .append($('<td>').append($('<button onclick="deletes_func(this.value)" > ')
                        .attr('id', i)
                        .attr('value', data[i].id)
                        .append('Delete'))));
